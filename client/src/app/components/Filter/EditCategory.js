...
const renderSmallField = ({ input, label, type, meta: { touched, error } }) => (
	<div>
		<div>
			<input style={{ fontSize: 11 }} {...input} type={type} placeholder={label} />
		</div>
	</div>
)
...
const renderSubCategories = ({ fields, meta: { error, submitFailed } }) => (
	<ul style={{ marginLeft: 40 }}>
		<li>
			<Button className="cyan darken-1" style={{ marginBottom: 25, fontSize: 10 }} type="button" onClick={() => fields.push({})}>
        add sub-category
			</Button>
		</li>
...
const renderCategories = ({ fields, meta: { error, submitFailed } }) => (
	<ul>
        ...
        <FieldArray name={`${category}.subCategories`} component={renderSubCategories} />
			</li>
		))}
    <li>
			<Button className="cyan darken-3" style={{ marginTop: 25 }} type="button" onClick={() => fields.push({})}>
        <Icon>add</Icon>
			</Button>
		</li>
	</ul>
		...
		return (
			<form onSubmit={handleSubmit}>
				<FieldArray name="categories" component={renderCategories} />
					<Button className="right" type="submit" disabled={submitting || pristine}>
            <Icon>save</Icon>
					</Button>
			</form>
		)
