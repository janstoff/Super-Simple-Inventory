...
class ItemsList extends Component {
	...
	render() {
		const { items, onViewItem } = this.props
		...
		return (
			<div>
				<p className="item-list-header" style={{ color: '#d4d2d2' }}>
					<i className="item-property" style={{ fontWeight: 600 }}>
						item
					</i>
					<i className="item-property">category</i>
					<i className="item-property">location</i>
					<i className="item-property">usage</i>
					<i className="item-quantity" style={{ fontWeight: 500 }}>
						quantity
					</i>
				</p>
				<Collection className="items-list">
					{items.map(item => {
						return (
							<Link
								to={`/items/${item._id}`}
							...
							>
								...
								>
									...
									<i className="item-property" style={{ color: '#8a9dbd' }}>
										{item.rental ? 'rental' : ''}
									</i>
									<i className="item-quantity" style={{ fontWeight: 500 }}>
										{item.quantity}
